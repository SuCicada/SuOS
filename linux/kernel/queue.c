...
// #define queue_real_end (queue->end + queue->capcity + 1) % (queue->capcity + 1)
#define queue_real_index(x) (x + queue->capcity) % (queue->capcity)
...
int queue_push(Queue* queue, unsigned char c) {
    if (!queue_full(queue)) {
        ...
        queue->end = queue_real_index(queue->end + 1);// + queue->capcity) % (queue->capcity);
        queue->size++;
        return 1;
    }
    return 0;
...
unsigned char queue_pop(Queue* queue) {
    if (!queue_empty(queue)) {
        ...
        // printf("%d\n",queue->node_arr[queue->start]);
        // printf("%d %c\n",c,c);
        // queue->start = (queue->start + 1 + queue->capcity + 1) % (queue->capcity + 1);
        queue->start = queue_real_index(queue->start + 1);// (queue->start + 1 + queue->capcity) % (queue->capcity);
        queue->size--;
        return c;
    }
    return 0;
